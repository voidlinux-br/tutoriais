...
# 🔥 防火墙 + 代理集成到 SAMBA4 域 (VOIDBR.ORG)

## 🎯 本教程的目标是在 **Debian 13** 上配置 **防火墙 + Squid 代理** 服务器，充当 **主网关 (192.168.70.254)** 并集成到 **VOIDBR.ORG** 域（域控制器：**192.168.70.253**），通过 **Active Directory (NTLM)** 对 Squid 中的用户进行身份验证并应用 **阻止政策**。
